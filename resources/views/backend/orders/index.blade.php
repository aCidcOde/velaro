@extends('layouts.backend', ['title' => 'Pedidos'])

@section('content')
    <div class="grid gap-4 py-4">
        <div>
            <h1 class="text-2xl font-semibold text-slate-900 dark:text-slate-100">Pedidos</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Gestão centralizada dos pedidos da base.</p>
        </div>
        ...
        <form method="GET" class="grid gap-3 rounded-4 border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-[1fr_220px_auto] dark:border-zinc-700 dark:bg-zinc-900">
            <input type="text" name="search" value="{{ $search }}" placeholder="Buscar pedidos" class="rounded-3 border border-slate-200 px-4 py-3 dark:border-zinc-700 dark:bg-zinc-950">
                ...
                <tbody>
                    @forelse ($orders as $order)
                        <tr class="border-t border-slate-200 dark:border-zinc-700">
                            ...
                            <td class="px-4 py-4 text-slate-600 dark:text-slate-300">{{ $order->user?->email ?? 'Sem conta' }}</td>
                            ...
                            <td class="px-4 py-4">
                                <div class="flex justify-end gap-3">
                                    <a href="{{ route('backend.orders.show', $order) }}" class="text-sm font-medium text-sky-600 dark:text-sky-300">Ver</a>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-10 text-center text-slate-500 dark:text-slate-400">Nenhum pedido encontrado.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
