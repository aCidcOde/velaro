@extends('layouts.backend', ['title' => 'Editar cliente'])
...
@section('content')
    <div class="rounded-4 border border-slate-200 bg-white p-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <form method="POST" action="{{ route('backend.customers.update', $customer) }}" class="mt-8 grid gap-6">
            @include('customers._form', ['customer' => $customer, 'method' => 'PUT'])

            <div class="flex justify-end gap-3">
                <a href="{{ route('backend.customers.show', $customer) }}" class="rounded-full border border-slate-200 px-5 py-3 text-sm font-semibold dark:border-zinc-700">Voltar</a>
